...
def test_multi_line_optimization():
    ...
    print("🔍 여러 라인을 활성화하여 생산능력을 확장하는 테스트 시작...")
    ...
    # 2. ProductionOptimizer 생성 (여러 라인 활성화)
    print("2️⃣ ProductionOptimizer 생성 (여러 라인 활성화)...")
    # 봉지면 생산 가능한 라인들: 12, 13 (라인 14는 컵면 전용)
    active_lines = ['12', '13']
    optimizer = ProductionOptimizer(ontology_manager, active_lines=active_lines)
    ...
    # 4. 모델 초기화
    print("4️⃣ 모델 초기화...")
    optimizer.model = LpProblem("Multi_Line_Production", LpMinimize)
    ...
    for time_slot in optimizer.ontology_timeslots:
        ...
        optimizer.model += total_production_time <= available_time * len(active_lines), f"time_limit_{time_slot}"
        print(f"      ✅ {time_slot}: {available_time}시간 × {len(active_lines)}라인 = {available_time * len(active_lines)}시간")
